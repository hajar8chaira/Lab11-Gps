# LAB 11 - Suivi GPS et Cartographie interactive (OpenStreetMap)
...
* **Géolocalisation asynchrone :** Maîtrise du service `LocationManager` et de la gestion des permissions au moment de l'exécution via `checkSelfPermission` et `onRequestPermissionsResult`.
* **Indépendance logicielle :** Remplacement réussi de l'écosystème propriétaire de Google Maps par une solution open-source robuste et autonome (osmdroid).
* **Gestion du cycle de vie des tuiles :** Mise en pause et réactivation du rendu visuel de la carte (`map.onResume()` / `map.onPause()`) en parfaite synchronisation avec le cycle de vie de l'activité Android.
